...
function(a){a.preventDefault();$("#avatar-modal").hide()});$(".smooth").smoothScroll({offset:-100});$(".comment-action").smoothScroll({afterScroll:function(){$("#comment").focus()}});$("#main-nav-toggle").on("click",function(){$(".inner-bar").slideToggle(200)});$("#comment").on("focus",function(){$(this).next(".comment-submit").show().find("#submit").attr("disabled",true)}).on("input",function(){var a=$(this).val();$(this).next(".comment-submit").find("#submit").attr("disabled",a==="")});$(".autosize").autosize({append:"\n\n\n"});
...
$(this).parent().addClass("subtle-border")});$(".tabs").tabs({tabContainer:".tab-container > div",activeClass:"tab-current"});$.getJSON("https://api.twitter.com/1/statuses/user_timeline/chalmersit.json?callback=?",function(a){var h=$("<ul />",{"class":"list"});if(a!=null){$.each(a,function(){var b=new Date(this.created_at);b="<p>"+Chalmers.linkify(this.text)+"</p><time>"+b.toDateString()+"</time>";b=$("<li />",{html:b});h.append(b)});$("#tweet-list").append(h)}else h.html("<li>Kunde inte h\u00e4mta tweets fr\u00e5n Twitter</li>")})});
