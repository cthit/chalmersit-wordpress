...
$(function(){$(".home #login-btn").on("click",function(b){b.preventDefault();$(".user-area form").jb_animate("shake").find("input[type='text']:first").focus()});$("#user-avatar-link").on("click",function(b){b.preventDefault();b=$("<iframe />",{src:this.href,scrolling:"no",id:"avatar-iframe",frameborder:"no",allowTransparency:"true"});$("#avatar-modal").append(b).modal("show");b.on("load",function(){$(this).contents().find("#user-avatar-step3-close").removeAttr("onclick").on("click",function(d){d.preventDefault();
$("#avatar-modal").modal("hide")})})});$("#avatar-modal").on("hidden",function(){$(this).find("iframe").remove()});$(".smooth").smoothScroll({offset:-100});$(".comment-action").smoothScroll({afterScroll:function(){$("#comment").focus()}});$("#main-nav-toggle").on("click",function(){$(".inner-bar").slideToggle(200)});$("#comment").on("focus",function(){$(this).next(".comment-submit").show().find("#submit").attr("disabled",true)}).on("input",function(){var b=$(this).val();$(this).next(".comment-submit").find("#submit").attr("disabled",
b==="")});$(".autosize").autosize({append:"\n\n\n"});$.fn.tipsy.elementOptions=function(b,d){return $.extend({},d,{gravity:$(b).data("tooltip-gravity")||"s",offset:parseInt($(b).data("tooltip-offset"))||0})};"ontouchstart"in document&&$("#tools-menu-trigger").on("touchstart",function(){var b=$(this).next(".dropdown-sub");b.hasClass("open")?b.removeClass("open").slideUp(100):b.addClass("open").slideDown(200)});$('[rel="tooltip"]').tipsy({gravity:"s",offset:5});$("article .article-content img:not(.avatar)").each(function(){$(this).parent().is("figure")&&
$(this).parent().addClass("subtle-border")});$(".tabs").tabs({tabContainer:".tab-container > div",activeClass:"tab-current"});$("[role='search'] input").on("keyup",function(b){b.which==27&&this.blur()});var a=$("meta[name=twitter-content]").attr("content"),h=$("meta[name=twitter-type]").attr("content"),c=$("meta[name=twitter-count]").attr("content"),e="https://";if(h==="user")e+="api.twitter.com/1/statuses/user_timeline/";e+=a+".json?callback=?";$.getJSON(e,function(b){var d=$("<ul />",{"class":"list"});
if(b!=null){$.each(b,function(f){var g=new Date(this.created_at);g="<p>"+Chalmers.linkify(this.text)+"</p><time>"+g.toDateString()+"</time>";g=$("<li />",{html:g});d.append(g);return f<c-1});$("#tweet-list").append(d)}else d.html("<li>Kunde inte h\u00e4mta tweets fr\u00e5n Twitter</li>")})});
