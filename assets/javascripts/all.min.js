...
$.fn.tipsy.elementOptions=function(a,h){return $.extend({},h,{gravity:$(a).data("tooltip-gravity")||"s",offset:parseInt($(a).data("tooltip-offset"))||0})};$('[rel="tooltip"]').tipsy({gravity:"s",offset:5});$("article .article-content img:not(.avatar)").each(function(){$(this).parent().is("figure")&&$(this).parent().addClass("subtle-border")});$(".tabs").tabs({tabContainer:".tab-container > div",activeClass:"tab-current"});$.getJSON("https://api.twitter.com/1/statuses/user_timeline/chalmersit.json?callback=?",
function(a){var h=$("<ul />",{"class":"list"});if(a!=null){$.each(a,function(){var b=new Date(this.created_at);b="<p>"+Chalmers.linkify(this.text)+"</p><time>"+b.toDateString()+"</time>";b=$("<li />",{html:b});h.append(b)});$("#tweet-list").append(h)}else h.html("<li>Kunde inte h\u00e4mta tweets fr\u00e5n Twitter</li>")})});
